...
public class SmartPickVideo extends StandardGSYVideoPlayer {
    ...
    private List<DetailBean.TrailersBean> mUrlList = new ArrayList<>();
...
//                    showSwitchDialog();
     ...
     * @return
     */
    public boolean setUp(List<DetailBean.TrailersBean> url, boolean cacheWithPlay, String title) {
        mUrlList = url;
        return setUp(url.get(mSourcePosition).getResource_url(), cacheWithPlay, title);
    }
     ...
     * @return
     */
    public boolean setUp(List<DetailBean.TrailersBean> url, boolean cacheWithPlay, File cachePath, String title) {
        mUrlList = url;
        return setUp(url.get(mSourcePosition).getResource_url(), cacheWithPlay, cachePath, title);
    }
...
//    }
    ...
    private void resolveStartChange(int position) {
        ...
        if (mSourcePosition != position) {
            if ((mCurrentState == GSYVideoPlayer.CURRENT_STATE_PLAYING
                ...
                showLoading();
                final String url = mUrlList.get(position).getResource_url();
                cancelProgressTimer();
                hideAllWidget();
    ...
    private void resolveChangedResult() {
        ...
        final String url = mUrlList.get(mSourcePosition).getResource_url();
        mTypeText = name;
        mSwitchSize.setText(name);
        resolveChangeUrl(mCache, mCachePath, url);
        hideLoading();
